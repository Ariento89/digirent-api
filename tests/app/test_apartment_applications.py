...
def test_landlord_reject_new_apartment_application_ok(
    ...
    tenant_application = application.reject_tenant_application(
        session, new_apartment_application.apartment.landlord, new_apartment_application
    )
...
def test_tenant_sign_contract_for_move_in_process_ok(
    new_contract: Contract, application: Application, session: Session
):
    assert new_contract.status == ContractStatus.NEW
    assert not new_contract.tenant_has_signed
    contract: Contract = application.tenant_signed_contract(session, new_contract)
    assert contract.tenant_has_signed
    assert contract.status == ContractStatus.NEW
...
def test_landlord_sign_contract_for_move_in_process_ok(
    ...
    assert new_contract.status == ContractStatus.NEW
    assert not new_contract.tenant_has_signed
    contract: Contract = application.landlord_signed_contract(session, new_contract)
    assert contract.landlord_has_signed
    assert contract.status == ContractStatus.NEW
...
def test_tenant_sign_already_signed_contract_fail():
    raise Exception()


def test_landlord_sign_contract_for_application_not_in_processing_status_fail():
    raise Exception()


def test_landlord_sign_already_signed_contract_fail():
    raise Exception()
...
def test_landlord_reject_a_completed_application_fail():
    raise Exception()
